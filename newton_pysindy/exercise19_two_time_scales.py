...
def stlsq_two_state(dt, minutes=42, threshold=0.01):
    ...
    model = ps.SINDy(differentiation_method=ps.SINDyDerivative(kind="finite_difference", k=1),
                     feature_library=ps.PolynomialLibrary(degree=2),
                     optimizer=ps.STLSQ(threshold=threshold, alpha=0, unbias=False,
                                        ridge_kw={"solver": "svd"}))
    model.fit(U, t=dt, feature_names=["u1", "u2"])
    for label, eq in zip(["u1'", "u2'"], model.equations(precision=4)):
        print(f"    dt = {dt:<5} {label} = {eq}")
